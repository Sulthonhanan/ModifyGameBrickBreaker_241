...
class GameObject:
    def _init_(self, canvas, x, y, width, height, color):
        self.canvas = canvas
...
class Ball(GameObject):
    def _init_(self, canvas, x, y, speed):
        super()._init_(canvas, x, y, 10, 10, 'red')
        self.speed = speed
        self.dx = speed
        self.dy = -speed
...
class Paddle(GameObject):
    def _init_(self, canvas, width):
        self.canvas_width = canvas.winfo_width()
        super()._init_(canvas, self.canvas_width/2 - width/2, 350, width, 10, 'blue')
        self.speed = 6
        self.canvas = canvas
...
class Brick(GameObject):
    def _init_(self, canvas, x, y, hits_to_break, color):
        super()._init_(canvas, x, y, 50, 20, color)
        self.hits_to_break = hits_to_break

class Game(tk.Frame):
    def _init_(self, master, difficulty="medium"):
        super()._init_(master)
        self.master = master
